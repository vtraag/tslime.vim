...
function! AlterTmuxText(text)
  let text = a:text
 
  " Make sure the text ends with a newline
  if strpart(text, strlen(text) - 1, 1) != "\n"
    let text = text . "\n"
  end
  return text
endfunction
...
function! SendToTmux(text)
    ...
    if (index(names, b:tslime['session']) < 0) " If session not in list
      call <SID>TmuxVars() " Set a new session
    end
    let text = AlterTmuxText(a:text)
    let oldbuffer = system(shellescape("tmux show-buffer"))
    echo text
    call <SID>SetTmuxBuffer(text)
...
nmap <unique> <Plug>NormalModeSendToTmux vip <Plug>SendSelectionToTmux }j
...
vmap <C-CR> <Plug>SendSelectionToTmux

" Key definition for SendSelectionToTmux() <Plug>.
nmap <C-CR> <Plug>NormalModeSendToTmux

" Key definition for SetTmuxVars() <Plug>
nmap <Leader>nt <Plug>SetTmuxVars
